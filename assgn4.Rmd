...
Next, Welch's t-tests were performed and Cohen's d statistics were computed (both at the 95% confidence level) in order to more formally explore the differences in mean sizes between years and levels of protection. The results of this test are shown below in Figure XX.
```{r}
# The results from the tests performed in the previous chunk are stored in a data frame and then displayed as a table
testtype <- c("MPA Sites: '12 v. '18", "Non-MPA Sites: '12 v. '18", '2012: MPA v. Non-MPA', '2018: MPA v. Non-MPA')
pvals <- c(t.mpa$p.value, t.nonmpa$p.value, t.2012$p.value, t.2018$p.value)
pvals <- round(pvals, 4)
...
testsummary.df <- tibble(testtype, diffs, pvals, ds)

testsummary.df %>% 
  kable(col.names = c('Test Type', 'Difference in Means', 'p-value', "Cohen's D"),
        align = c('l','c','c','c'),
        caption = 'Figure XX: Results of statistical analysis    ') %>% 
  kable_styling(bootstrap_options = c("striped",'bordered','condensed'),
                full_width = F,
                position = "center") %>% 
  add_header_above(c("Lobster Size Test Results" = 4),
                   bold = T,
                   color = 'white',
                   background = 'grey')
