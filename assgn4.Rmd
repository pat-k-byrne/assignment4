...
```
Turning from a visual exploration to a more quantitative one, mean and standard deviations of lobster sizes at MPA and non-MPA sites were computed for 2012 and 2018. The values calculated can be found in Figure XX to the right. A brief examination confirms that they are consistent with expectations based on Figure XX. Mean size increased for MPA sites but not for non-MPA sites, the standard deviation decreased for non-MPA sites, and the difference in sample sizes is consistent with the trends in overall abundance shown in Figure XX. Furthermore, examining the sample sizes confirms what Figure XX suggests, that abundance of spiny lobsters increased more between 2012 and 2018 at MPA sites than it did at non-MPA sites
...
Next, Welch's t-tests were performed and Cohen's d statistics were computed (both at the 95% confidence level) in order to more formally explore the differences in mean sizes between years and levels of protection. The results of these tests are shown below in Figure XX.
```{r}
...
testsummary.df %>% 
  kable(col.names = c('Test Type', 'Difference in Means (mm)', 't', 'df', 'p-value', "Cohen's D"),
        align = c('l','c','c','c'),
        caption = "Figure XX: Results of t-tests and Cohen's d calculations. t refers to the calculated t-statistic, and df refers to the degrees of freedom.") %>% 
  kable_styling(bootstrap_options = c("striped",'bordered','condensed'),
                full_width = F,
                position = "center") %>% 
